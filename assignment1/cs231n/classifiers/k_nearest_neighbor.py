...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in tqdm(range(num_test)):
      ...
      dists[i, :] = np.linalg.norm(X[i]-self.X_train, ord=2, axis=1) # no order
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in tqdm(range(num_test)):
      ...
      labels, counts = labels[count_sort_ind], counts[count_sort_ind]
      if len(labels) == 1 or counts[0] > counts[1]:
            y_pred[i] = labels[0]
      else:
        y_pred[i] = labels[0] if labels[0] < labels[1] else labels[1]
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
